...
TITAN_EMBED_MODEL_ID=${TITAN_EMBED_MODEL_ID:-}
CLAUDE_MODEL_ID=${CLAUDE_MODEL_ID:-}
COMMERCE_GUARDRAIL_ENABLED=${COMMERCE_GUARDRAIL_ENABLED:-}
...
if [ ! -z "$CATALOG_BUCKET" ]; then
    CDK_CONTEXT="$CDK_CONTEXT -c catalogBucket=$CATALOG_BUCKET"
fi
if [ ! -z "$CATALOG_PREFIX" ]; then
    CDK_CONTEXT="$CDK_CONTEXT -c catalogPrefix=$CATALOG_PREFIX"
fi
if [ ! -z "$TITAN_EMBED_MODEL_ID" ]; then
    CDK_CONTEXT="$CDK_CONTEXT -c embedModel=$TITAN_EMBED_MODEL_ID"
fi
if [ ! -z "$CLAUDE_MODEL_ID" ]; then
    CDK_CONTEXT="$CDK_CONTEXT -c chatModel=$CLAUDE_MODEL_ID"
fi
if [ ! -z "$COMMERCE_GUARDRAIL_ENABLED" ]; then
    CDK_CONTEXT="$CDK_CONTEXT -c commerceGuardrailEnabled=$COMMERCE_GUARDRAIL_ENABLED"
fi
